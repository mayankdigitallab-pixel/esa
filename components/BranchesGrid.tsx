...
export function BranchCard({ centre: c, showLandingLink = true }: CardProps) {
  return (
...
export function BranchesGrid({ centres, showLandingLink = true }: GridProps) {
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {centres.map((c) => (
        <BranchCard key={c.slug} centre={c} showLandingLink={showLandingLink} />
      ))}
    </div>
  );
}
